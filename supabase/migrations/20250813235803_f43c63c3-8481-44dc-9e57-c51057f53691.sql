...
DROP POLICY IF EXISTS "Users can create revines" ON public.revines;
DROP POLICY IF EXISTS "Users can delete their own revines" ON public.revines;
DROP POLICY IF EXISTS "Users can view revines" ON public.revines;
...
-- Create revines table if not exists
...
-- Create saved_videos table if not exists
...
-- Drop existing policies for saved_videos if they exist
...
DROP POLICY IF EXISTS "Users can view their saved videos" ON public.saved_videos;
...
DROP POLICY IF EXISTS "Users can view collection items based on collection privacy" ON public.collection_items;
...
-- Create trigger for collections updated_at (if not exists)
...
-- Create trigger for revines count updates (if not exists)
